...
router.post('/get-debt', async (req, res) => {
  const { name } = req.body;
  try {
    const resp = await client.query(`SELECT * FROM purchase_debt WHERE supplier='${name}'`);
    if (resp.rows.length === 0) {
      let result = [
        {
          "id": 0,
...
router.post("/get-debt-date", async (req, res) => {
  ...
  try {
    const resp = await client.query(`SELECT * FROM purchase_debt WHERE supplier='${supplier}' AND recorded_date LIKE '%${date}%'`);
    if (resp.rows.length === 0) {
      let result = [
        {
          "id": 0,
